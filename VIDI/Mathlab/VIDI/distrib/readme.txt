    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_VIDI.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: VIDI.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * VIDI.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * VIDI.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * VIDI (application created by MATLAB Compiler)
        * run_VIDI.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * VIDI.dll (shared library, file extension varies by 
          platform)
        * VIDI.h (library header file)
    ...
    Windows:
        * VIDI.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * VIDI.xml (documentation files)
        * VIDI.pdb (program database file - if DEBUG option 
          selected)
        * VIDI.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * VIDI.jar (Java class files and compiled M-code)
    ...
    Windows:
        * VIDI.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * VIDI_1_0.dll (COM component)
        * VIDI.xla (optionally created and placed in
          C:\VIDI\Matlab\VIDI\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
